...
namespace Hunter.NATS.Client
{
    ...
    public sealed class AsyncSubscription : Subscription, IAsyncSubscription, ISubscription
    {
        ...
        /// </summary>
        public event EventHandler<MessageHandlerEventArgs> MessageHandler;

        private Task                msgFeeder = null;

        private bool started = false;

        internal AsyncSubscription(NATSConnection conn, string subject, string queue)
            : base(conn, subject, queue)
        {
            //TODO:获取消息通道
            mch = null;//conn.getMessageChannel();
            if ((ownsChannel = (mch == null)))
            {
                mch = new Channel<Message>()
                {
        ...
        internal override bool processMsg(Message msg)
        {
            NATSConnection localConn;
            EventHandler<MessageHandlerEventArgs> localHandler;
            long localMax;
            long d;
            ...
            if (localMax <= 0 || d <= localMax)
            {
                try
                {
                    ...
                    {
                        var msgHandlerEventArgs = new MessageHandlerEventArgs(msg);
                        //TODO:已经优化
        ...
        /// method is a no-op.
        /// </remarks>
        /// <exception cref="NATSBadSubscriptionException">There is no longer an associated <see cref="NATSConnection"/>
        ...
        /// <see cref="NATSConnection"/>'s Request methods use this functionality.</para>
