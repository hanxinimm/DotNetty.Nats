...
namespace Hunter.NATS.Client
{
    ...
    public class Subscription : ISubscription, IDisposable
    {
        ...
        /// <summary>
        /// 启用自动取消订阅时,设置的处理消息的最大值
        /// </summary>
        public int MaxMessage { get; set; } = -1;
        ...
        internal NATSConnection conn = null;
        internal bool closed = false;
        internal bool connClosed = false;

        internal Channel<Message> mch = null;
        ...
        internal long pMsgsLimit = DefaultsOptions.SubPendingMsgsLimit;
	    internal long pBytesLimit = DefaultsOptions.SubPendingBytesLimit;
        ...
        internal Subscription(NATSConnection conn, string subject, string queue)
        {
            this.conn = conn;
        ...
        {
            if (MaxMessage > 0 && msgs > MaxMessage)
                return true;
        ...
        /// <summary>
        /// Called by <see cref="Hunter.NATS.Client.NATSConnection"/> when a <see cref="Message"/> is received, returning
        /// a value indicating if the <see cref="Hunter.NATS.Client.NATSConnection"/> should keep the subscription
        /// after processing.
        /// </summary>
        /// <param name="msg">A <see cref="Message"/> received by the <see cref="Subscription"/>.</param>
        /// <returns><c>true</c> if-and-only-if the <see cref="Subscription"/> should remain active;
        /// otherwise <c>false</c> if the <see cref="Hunter.NATS.Client.NATSConnection"/> should remove this
        /// instance.</returns>
        internal virtual bool processMsg(Message msg)
        {
            return true;
        }

        private void handleSlowConsumer(Message msg)
        {
            dropped++;
            //TODO:处理慢速消费
        ...
        /// <param name="msg">The <see cref="Message"/> object delivered to a
        ...
        // to the channel.
        internal bool addMessage(Message msg, int maxCount)
        {
