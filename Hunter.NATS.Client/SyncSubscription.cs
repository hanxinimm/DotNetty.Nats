...
namespace Hunter.NATS.Client
{
    ...
    {
        internal SyncSubscription(NATSConnection conn, string subject, string queue)
            : base(conn, subject, queue)
        {
            mch = new Channel<Message>()
            {
                Name = subject + (String.IsNullOrWhiteSpace(queue) ? string.Empty : string.Format(" (queue: {0})", queue)),
            };
        }
        ...
        /// <see cref="Message"/>.</exception>
        public Message NextMessage(int timeout)
        {
            ...
            lock (mu)
            {
                ...
                else if (MaxMessage > 0 && delivered >= MaxMessage)
                {
